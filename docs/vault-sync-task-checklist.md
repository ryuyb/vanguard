...
- [x] 为 `Type=0/1/2` 增加按 `Payload.Id` 的条目级更新能力
- [x] 为 `Type=3/7/8` 增加文件夹级局部刷新
- [x] 为 `Type=12/13/14` 增加 Send 局部刷新
- [x] 保留“失败回退全量同步”兜底路径

### 性能优化

- [x] 支持批量写入与分批事务（大库场景）
- [x] 减少不必要字段写入（revision 未变化跳过）
- [x] 同步结果缓存统计（耗时、对象数量、失败率）

### 稳定性与安全

- [x] 附件 URL 只短期使用，不长期缓存
- [x] 日志脱敏（不输出 token、password、masterPasswordHash）
- [x] 关键路径超时控制（HTTP、WS、DB）
- [x] 加入异常注入测试（网络抖动、WS 闪断、DB 锁冲突）
