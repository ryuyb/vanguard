...
fn normalize_base_url(base_url: &str) -> AppResult<String> {
    ...
    if normalized.is_empty() {
        return Err(AppError::validation("base_url cannot be empty"));
    }
    Ok(normalized)
}
...
fn decode_token_subject(access_token: &str) -> AppResult<String> {
    let payload_segment = access_token
        ...
        .split('.')
        .nth(1)
        .ok_or_else(|| AppError::validation("access_token is not a valid JWT"))?;

    let decoded = URL_SAFE_NO_PAD
        .decode(payload_segment)
        .or_else(|_| URL_SAFE.decode(payload_segment))
        .map_err(|_| AppError::validation("access_token payload is not valid base64url"))?;

    let claims: Value = serde_json::from_slice(&decoded)
        .map_err(|_| AppError::validation("access_token payload is not valid JSON"))?;
    ...
    let subject = claims
        ...
        .ok_or_else(|| AppError::validation("access_token payload missing subject claim"))?;
