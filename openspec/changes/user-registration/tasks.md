...
- [ ] 1.1 添加 Rust 密码学依赖 (ring, rsa, base64)
- [ ] 1.2 创建注册功能模块目录结构
- [ ] 1.3 更新 i18n 资源文件,添加注册相关文案
...
- [ ] 3.1 创建 `/register` 路由
- [ ] 3.2 创建 `register-page.tsx` 注册页面组件
- [ ] 3.3 创建 `register-form.tsx` 注册表单组件
- [ ] 3.4 实现表单验证 schema (邮箱、服务器 URL、名字)
- [ ] 3.5 创建 `registration-feedback.tsx` 反馈提示组件
- [ ] 3.6 在登录页添加"创建新账户"导航链接

## 4. 注册流程逻辑 (阶段 1: MVP)

- [ ] 4.1 创建 `use-registration-flow.ts` hook
- [ ] 4.2 实现场景 1: 处理 400 错误响应,显示错误提示
- [ ] 4.3 实现场景 2: 处理 204 响应,显示邮件已发送提示
- [ ] 4.4 实现场景 3 占位: 处理 200 响应,显示"开发中"提示
- [ ] 4.5 实现返回登录页面功能
- [ ] 4.6 添加加载状态和进度提示
